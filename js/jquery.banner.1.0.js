...
//				根据鼠标点击的当前项的索引,添加当前项的样式
				options.list.eq($(this).index()).css({backgroundColor:"red"}).siblings().css({backgroundColor:""})
			})
...
//				console.log(that.LOCAL.iPrev,that.LOCAL.index)
			...
			options.right.on("click",that.LOCAL.right)
...
//		左右按钮的move，根据正负号决定方向
		that.LOCAL.btnMove = function(type){
			...
			if(that.LOCAL.listOnoff){
				options.list.eq(that.LOCAL.index).css({backgroundColor:"red"}).siblings().css({backgroundColor:""})
			}
		}
